...
export function AdminDashboard() {
  const { students, teachers, courses, fees } = useDataStore();

  const totalRevenue = fees.filter((f) => f.status === 'paid').reduce((a, f) => a + f.paid, 0);
  const pendingFees = fees.filter((f) => f.status !== 'paid').reduce((a, f) => a + f.due, 0);
